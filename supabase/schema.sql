...
create index if not exists idx_club_support_threads_club_last_message on public.club_support_threads(club_id, last_message_at desc, created_at desc);
create index if not exists idx_club_support_threads_user_last_message on public.club_support_threads(user_id, last_message_at desc, created_at desc);
create index if not exists idx_club_support_messages_thread_created on public.club_support_messages(thread_id, created_at);
create index if not exists idx_club_support_messages_club_created on public.club_support_messages(club_id, created_at desc);
...
values
  ...
  (
    'booking-schedule',
    array['booking','book','slot','schedule','reservation','availability','available seat','seats']::text[],
  ...
  (
    'payment-refund',
    array['pay','payment','fee','refund','order','charge','billing']::text[],
  ...
  (
    'maps-location',
    array['map','location','address','venue','place','directions']::text[],
  ...
  (
    ...
    array['register','registration','publish','approved','review','approval','submit']::text[],
  ...
  (
    ...
    array['teaching','class','classes','lesson','lessons','learn','learning','coach','instructor','teacher','teaching content','teaching method','teaching methods','method','methods','content','contents','item','items','syllabus','topic','topics','what learn','curriculum','lesson plan']::text[],
  ...
  (
    'club-course-info',
    array['club','course','info','information','detail','details','about','introduction']::text[],
  ...
  (
    'forum-community',
    array['forum','post','posts','community','comment','comments','thread','threads','discussion','activity']::text[],
  ...
  (
    'cancel-reschedule',
    array['cancel','cancellation','reschedule','change booking','change slot','change time','switch time']::text[],
  ...
  (
    'dashboard-records',
    array['dashboard','record','records','history','my booking','bookings','booking history','support history']::text[],
  ...
  (
    'attachments-proof',
    array['attachment','attachments','upload','image','images','screenshot','screenshots','file','files','photo','photos']::text[],
  ...
  (
    'login-account',
    array['login','log in','sign in','signup','sign up','account','password','register account','credentials']::text[],
